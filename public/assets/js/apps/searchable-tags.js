...
const tagInput = document.querySelector(".searchable-tag-input");
const tagArea = document.querySelector(".searchable-tag-area");
const ul = document.querySelector(".searchable-tag-area ul");
const label = document.querySelector(".searchable-tag-label");
const tagsInput = document.querySelector("#searchable-tags-input");
...
function addEvent(element) {
    ...
    element.addEventListener("focus", () => {
        ...
        label.classList.add("searchable-tag-label-active");
    });

    element.addEventListener("blur", (e) => {
        tagArea.classList.remove("active");
        if (element.value === "" && tags.length === 0) {
            label.classList.remove("searchable-tag-label-active");
        }
...
function renderTags() {
    ...
    input.type = "text";
    input.className = "searchable-tag-input";
    addEvent(input);
